...
Row & Layer Management: Add regular or half-size rows, reorder them via drag-and-drop, and manage layouts using the dedicated Layers panel.

Shape Customization & Uploads: Choose from built-in vector shapes (petals, circles, ovals, stars, lines) or upload custom PNG/JPEG images to use as components. 

Color Palette: Pick from pre-set color options, add custom colors with a built-in color picker, and rename or delete palette items.

Controls & Scaling: Adjust cell counts, overall item sizing (XS to XL), horizontal/vertical scaling (Scale X/Y), and custom spacing per row.
